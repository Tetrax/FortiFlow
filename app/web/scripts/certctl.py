#!/usr/bin/env python3
"""Install and validate TLS certificates for FortiFlow."""
...
def validate_chain(leaf: bytes, chain: list[bytes]) -> list[bytes]:
    ...
    with tempfile.TemporaryDirectory(prefix="fortiflow-chain-") as temporary:
...
def configured_runtime_owner() -> tuple[int, int] | None:
    if os.geteuid() != 0:
        return None
    if not (os.environ.get("PUID") or os.environ.get("PGID")):
        try:
            runtime = pwd.getpwnam("fortiflow")
        except KeyError:
            return None
...
def install_certificate(
    ...
    temporary_dir = Path(tempfile.mkdtemp(prefix="fortiflow-certctl-"))
    activated = False
    try:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Installer un certificat TLS pour FortiFlow."
    )
