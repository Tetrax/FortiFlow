'use client'
// Formulaire de soumission d'une idée
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label htmlFor="title" className="block text-sm font-medium text-white mb-1">
          Titre de l'idée <span className="text-red-400">*</span>
        </label>
        <input
          id="title"
          name="title"
          ...
          required
        />
        <p className="mt-1 text-xs text-[#9CA3AF]">{form.title.length}/150 caractères</p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          required
        />
        <p className="mt-1 text-xs text-[#9CA3AF]">{form.description.length}/2000 caractères</p>
      </div>
      ...
      {/* Anonymat */}
      <div className="bg-[#1e1040] border border-[#6B21E8]/30 rounded-lg p-4">
        <label className="flex items-start gap-3 cursor-pointer">
          <input
            type="checkbox"
            checked={isAnonymous}
            onChange={(e) => setIsAnonymous(e.target.checked)}
            className="mt-0.5 h-4 w-4 rounded border-[#1F2937] text-[#6B21E8] focus:ring-[#6B21E8] bg-[#0A0A0A]"
          />
          <div>
            <span className="text-sm font-medium text-white">Soumettre anonymement</span>
      ...
      {!isAnonymous && (
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="authorEmail" className="block text-sm font-medium text-white mb-1">
              Votre email
            </label>
            <input
              id="authorEmail"
      ...
      <p className="text-xs text-[#9CA3AF] text-center">
        ...
        <a href="/rgpd" className="underline hover:text-white transition-colors">
          politique RGPD
        </a>
        .
      </p>
